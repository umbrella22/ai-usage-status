...
- **多供应商支持** — 目前支持 MiniMax（国内/海外）和 智谱/ZAI 两个 AI 平台
...
- 支持 ZAI（api.z.ai）和 智谱（open.bigmodel.cn）平台
- 显示 Token 用量配额、MCP 工具调用配额
- 24 小时 Token 消耗和工具调用统计

## 快速开始

1. 首次安装后会自动打开使用指引，说明如何配置和查看状态
...
| 设置项                          | 说明                     |
| ------------------------------- | ------------------------ |
| `aiUsageStatus.zhipu.enabled`   | 启用智谱 / ZAI           |
| `aiUsageStatus.zhipu.authToken` | 认证 Token               |
| `aiUsageStatus.zhipu.baseUrl`   | 平台地址（ZAI/智谱）     |
| `aiUsageStatus.zhipu.modelName` | 模型选择（留空自动获取） |

## 命令

- `AI Usage Status: 首次使用指引` — 重新打开首次安装引导页
